...
class ModulusUnoService {
  ...
  def approveCashOutOrder(CashOutOrder cashOutOrder) {
    ...
    CashoutCommand command = new CashoutCommand(uuid:cashOutOrder.timoneUuid, clabe:cashOutOrder.account.clabe, bankCode:cashOutOrder.account.banco.bankingCode, amount:amount, fee:feeCommand.amount, beneficiary:cashOutOrder.company.bussinessName, concept:"${cashOutConcept.CashOutOrder} ${cashOutOrder.id}", feeType:feeCommand.type)
