...
class SaleOrderServiceSpec extends Specification {
  ...
  void "Should get the sale orders to list for a user with associate business entities groups"() {
    given:"The company"
      Company company = new Company().save(validate:false)
    ...
    and:"The users clients"
      businessEntitiesGroupService.findClientsGroupsForUserInCompany(_, _) >> [Mock(BusinessEntitiesGroup)]
      businessEntitiesGroupService.getAllClientsFromUserGroups(_) >> [new BusinessEntity(rfc:"AAA").save(validate:false), new BusinessEntity(rfc:"BBB").save(validate:false), new BusinessEntity(rfc:"CCC").save(validate:false)]
    and:
      User user = Mock(User)
